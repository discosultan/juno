...
fn map_period_deltas_from_summary(
    ...
    for pos in summary.positions.iter() {
        ...
        // Open.
        let time = time.floor_multiple(interval);
        ...
        // Close.
        let time = close_time.floor_multiple(interval);
        let deltas = period_deltas
            .entry(time)
            .or_insert_with(Vec::<(Asset, f64)>::new);
...
fn get_asset_performance(
    ...
    let start = summary.start.floor_multiple(interval);
    let end = summary.end.floor_multiple(interval);
...
pub fn get_sharpe_ratio(
    ...
    // TODO: Implement Step trait for Interval once stable.
...
mod tests {
    ...
    fn test_nonoptimized_stats_same_as_optimized() {
        ...
        let stats = ExtendedStatistics::compose(&summary, &base_prices, None, Interval(1));

        let opt_sharpe = get_sharpe_ratio(&summary, &base_prices, None, Interval(1));
        let opt_sortino = get_sortino_ratio(&summary, &base_prices, None, Interval(1));
