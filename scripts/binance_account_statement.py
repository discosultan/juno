...
parser.add_argument(
    "--account-number",
    default=None,
    help="Optional account number to put on the account statement.",
)
parser.add_argument(
    "--account-date",
    default=None,
...
async def main() -> None:
    ...
    storage = SQLite()
    exchange = Exchange.from_env("binance")
...
def generate_statement(
    ...
    account_number = f" {args.account_number}" if args.account_number else ""
    account_date = (
    ...
    lines.extend(
        [
            ...
            f"{account_number} with Binance.com.{account_date}",
        ]
    )
    ...
    lines.extend(
        [
            ...
            "available crypto-asset tokens with respect to Spot, Funding, Margin, Futures and "
            "Earn wallets.",
        ]
    )
    ...
    lines.extend(
        [
            "",
            "Overview",
            "Date\t\tUser ID\t\tAccount Type\tWallet\tRate",
            f"{format_timestamp(args.date)}\t{args.account_number}\tMasterAccount\tAll\t\tBTC 1 = EUR {btc_eur_rate.quantize(eur_precision)}",
        ]
    )
    ...
    # if any(balance for balance in balances if balance.eur_value.is_nan()):
    #     lines.extend(
    #         [
    #             "",
    #             f"Excluded Assets (price unavailable or value less than EUR {args.threshold})",
    #         ]
    #     )
    #     lines.extend(
    #         [
    #             f"{balance.asset.upper()} {format_decimal(balance.amount)}"
    #             for balance in balances
    #             if balance.eur_value.is_nan() or balance.eur_value < args.threshold
    #         ]
    #     )
