...
class Binance:
    ...
    async def get_account_info(self, symbol):
        ...
        return AccountInfo(time_ms(), base_balance, quote_balance,
                           Fees(res['makerCommission'] / 10000.0, res['takerCommission'] / 10000.0))

    async def place_order(self, symbol, side, type_, qty, price, time_in_force, test=False):
        test_suffix = '/test' if test else ''
        url = f'/api/v3/order{test_suffix}?symbol={_http_symbol(symbol)}&side={side.name}&type={type_.name}&quantity={qty}'
    ...
    async def get_trades(self, symbol):
        ...
        return [Trade(x['price'], x['qty'], x['commission'], x['commissionAsset'], x['isBuyer']) for x in result]
    ...
    async def stream_candles(self, symbol, interval, start, end):
        current = div_by_or_prev(time_ms(), interval)
        if start < current:
            async for candle, primary in self._stream_historical_candles(symbol, interval, start, min(end, current)):
    ...
    async def _stream_historical_candles(self, symbol, interval, start, end):
        ...
        for page_start, page_end in page(start, end, interval, MAX_CANDLES_PER_REQUEST):
            url = f'/api/v1/klines?symbol={_http_symbol(symbol)}&interval={_interval(interval)}&startTime={page_start}&endTime={page_end - 1}'
            res = await self._request('GET', url, 1)
            for c in res:
                yield Candle(c[0], float(c[1]), float(c[2]), float(c[3]), float(c[4]), float(c[5])), True
        ...
        while True:
            ...
            async with self._ws_connect(url) as ws:
                async for msg in ws:
                    ...
                    # Since updates are given every second, we are only interested in the last update
                    # for any particular candle. We keep track of two consecutive candles to find the
                    # last one for a period. Note that:
                    ...
                    if c.time >= end - self.interval:
                        return
                    if time_ms() - stream_start > MS_IN_12HOURS:
                        break
    ...
    async def _order(self, method, url, cost):
        # Synchronize clock. Note that we may want to do this periodically instead of only initially.
        ...
        m = hmac.new(self._secret_key, query_str, hashlib.sha256)
    ...
    async def _ensure_clock_synced(self):
        ...
        # task if the event loop ends before the task is finished. Otherwise, we will get a warning.
