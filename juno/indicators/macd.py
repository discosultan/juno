...
class Macd:
    ...
    def __init__(self, short_period, long_period, signal_period):
        ...
        if long_period < short_period:
            raise ValueError(f'long period ({long_period}) must be larger than or equal to short period ({short_period})')

        # A bit hacky but is what is usually expected.
        if short_period == 12 and long_period == 26:
            self.short_ema = Ema.with_smoothing(0.15)
            self.long_ema = Ema.with_smoothing(0.075)
        else:
    ...
    def update(self, price):
        ...
        macd, signal, divergence = None, None, None
