...
from juno.time import DAY_MS, strptimestamp, time_ms
...
class Foo(Agent):
    ...
    async def run(self) -> None:
        start = strptimestamp('2019-07-01')
        exchange = 'binance'

        tickers = [t for t in self.informant.list_tickers(exchange) if t.symbol.endswith('-btc')]
        tickers.sort(key=lambda t: t.volume, reverse=True)
        assert len(tickers) > 5
        tickers = tickers[:5]

        quote = Decimal('1.0')
        summary = TradingSummary(quote=quote)
        await asyncio.gather(*(optimize_and_trade(t.symbol, summary) for t in tickers))
    ...
    async def optimize_and_trade(symbol: str, summary: TradingSummary):
        pass
