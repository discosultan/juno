...
class Limit:
    ...
    async def buy(self, exchange: str, symbol: str, quote: Decimal, test: bool) -> OrderResult:
        ...
        res = await self._fill(exchange, symbol, Side.ASK, quote)
    ...
    async def sell(self, exchange: str, symbol: str, base: Decimal, test: bool) -> OrderResult:
        ...
        res = await self._fill(exchange, symbol, Side.BID, base)
        fees = self._informant.get_fees(exchange, symbol)
    ...
    async def _keep_limit_order_best(self, exchange: str, symbol: str, client_id: str, side: Side,
        ...
        try:
            ...
            last_order_price = Decimal(0) if side is Side.ASK else Decimal('Inf')
            while True:
                await orderbook_updated.wait()

                asks = self._orderbook.list_asks(exchange, symbol)
                bids = self._orderbook.list_bids(exchange, symbol)
                ob_side = bids if side is Side.ASK else asks
                ob_other_side = asks if side is Side.ASK else bids
                op_step = operator.add if side is Side.ASK else operator.sub
                op_last_price_cmp = operator.le if side is Side.ASK else operator.ge

                if len(ob_side) == 0:
                    raise NotImplementedError(
                        f'no existing {"asks" if side is Side.ASK else "bids"} in orderbook! what '
                        'is optimal price?')
                ...
                size = available / price if side is Side.ASK else available
