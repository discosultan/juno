...
@pytest.mark.parametrize(
    "value,multiple,expected_output",
    [
        (1, 5, 5),
        (5, 5, 5),
        (6, 5, 10),
...
def test_ceil_multiple_offset() -> None:
    assert math.ceil_multiple_offset(Decimal("4.00"), Decimal("2.00"), Decimal("1.00")) == Decimal(
        "5.00"
    )
...
@pytest.mark.parametrize(
    "total,parts,precision,expected_output",
    [
        (
            Decimal("1.0001"),
            4,
